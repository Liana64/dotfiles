...
- `modules/` — the dendritic tree. A leaf registers config under `flake.modules.<class>.<aspect>` (`class` = `nixos` | `homeManager`); the same aspect from many files merges. No `default.nix` aggregators — adding a file is enough. **Module index: `modules/AGENTS.md`** — consult it before exploring or fanning out search agents.
  ...
  - `modules/agentic/` — the Claude Code config module **and** its materialized content (`AGENTS.md` → `~/.claude/CLAUDE.md`, agents, skills). Owns all Claude Code config; `~/.claude` is a read-only materialization — change it here, applies after a home-manager switch. System map: `modules/agentic/AGENTS.md` — read it instead of exploring this module.
...
- `hosts/<name>/` — thin, NOT auto-imported: `hardware-configuration.nix` + `options.nix` (hostname, password, theme/compositor/taskManager); the shared home base (`hmShared`) lives in `modules/flake/hosts.nix`. Desktop hosts import *all* aspects and differ only via options (`framework`/`portable` are the same laptop; `noku` a draft TV box). A host entry in `hosts.nix` may instead name an `aspects` subset and set `home = false`: `m1` (headless ZFS hypervisor/NAS, GRUB + networkd, no home-manager) takes only `nixDaemon`/`time`/`users` and keeps its server config in `hosts/m1/` — ops doc `hosts/m1/README.md`. `oob` (rpi5 UniFi controller, `channel = nixpkgs-unstable`) follows the same pattern — ops doc `hosts/oob/README.md`.
