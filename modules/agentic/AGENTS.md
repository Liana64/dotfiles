...
- Track work as harness tasks (TaskCreate/TaskUpdate) and keep them current; "task" always means the harness, never the `/todo` skill.
...
- Never `git commit` or `git push` a project (`git add` is fine); the sole exception is the `/remember` skill committing and pushing the ai-memory repo.
